...
def construct_obs_plot(df_plot, adata, perc=0, suffix=None):
    # clip
    df_plot = df_plot.clip(df_plot.quantile(perc), df_plot.quantile(1 - perc), axis=1) # df_plot.quantile默认axis=0，即一列（一个基因）取一个quantile
    # 低于perc比例的、高于1-perc比例的，分别设置成perc比例和1-perc比例

    # normalize
    df_plot = (df_plot - df_plot.min()) / (df_plot.max() - df_plot.min()) # 每个基因内部，minmax归一化

    if suffix: # For DataFrame, the column labels are suffixed.
...
def construct_anndata_from_df_and_img(df, path_img, array_coords_for_df):
    '''
    df: expression dataframe of n_cells * n_genes. 
    path_img: path to the spatial matrix. 
    array_coords_for_df: numpy array of n_cells * 2, indicating the (x, y)s for each cell.
    '''
    # df = pd.DataFrame([[1,2,],[3,4,],[5,6]], index=['s1','s2','s3'], columns=['G1','G2'])
    adata = AnnData(df)

    img = cv2.imread(path_img)
    adata.uns['spatial'] = {'V1':{'images':{'hires':img}}}
    adata.obsm['spatial'] = array_coords_for_df # np.array([[1,2],[4,6],[8,3]])
...
def plot_genes_sc(
    ...
    bw=False,
    return_figure=False,
    kargs_for_sc_pl_spatial={}
):
    '''
    kargs_for_sc_pl_spatial: the kargs to pass to sc.pl.spatial, such as crop_coord.
    '''
    ...
    # prepare adatas
    convert_adata_array(adata_measured) # adata_measured.X如果是压缩的矩阵则展开成稀疏表示
    ...
    for ix, gene in enumerate(genes):
        if gene not in adata_measured.var.index:
            data.append(np.zeros_like(np.array(adata_measured[:, 0].X).flatten())) # 未出现在真实st中就认为全0
        else:
            data.append(np.array(adata_measured[:, gene].X).flatten())

    df = pd.DataFrame(
        data=np.array(data).T, columns=genes, index=adata_measured.obs.index,
    ) # 来自adata_measured的n_samples * n_genes
    construct_obs_plot(df, adata_measured, suffix="measured") 
    # 对df进行clip后归一化，并将df的基因名增加"(measured)"后缀后将df 在 concat到adata_measured.obs右侧。
    ...
    # adata_predicted.obsm['spatial']是一个测量点的(x, y)的矩阵。
    #non visium data
    ...
    for ix, gene in enumerate(genes):
        ...
        # sc.pl.spatial 是scanpy提供的绘图函数，https://blog.csdn.net/qq_40943760/article/details/125201948
        ...
        # Use the parameter img_key to see the image in the background And the parameter library_id to select the image. 
        # By default, 'hires' and 'lowres' are attempted.

        # crop_coord : Optional[Tuple[int, int, int, int]] (default: None)
        ...
        sc.pl.spatial(
            ...
            color=["{} (measured)".format(gene)], # 根据gene表达量进行颜色选择
            frameon=False,
            ax=ax_m,
            show=False,
            cmap=cmap,
